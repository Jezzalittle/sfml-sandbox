...
void Raycast::CheckForRaycollision(std::vector<Shape*> shapeArr)
{
	...
	for (size_t i = 0; i < shapeArr.size(); i++)
	{
		...
		for (int j = 0; j < shapeArr[i]->getVertex().size(); j++)
		{
			...
			if (LineSegmentIntersectionPoint(shapeArr[i]->getVertex()[0] + Vector2(-1.05, 0), shapeArr[i]->getVertex()[1] + Vector2(1.05 ,0), intersect))
			{
				intersectArr.push_back(intersect);
				hit = true;
			}
			if (LineSegmentIntersectionPoint(shapeArr[i]->getVertex()[1] + Vector2(0,1.05), shapeArr[i]->getVertex()[2] + Vector2(0, -1.05), intersect))
			{
				intersectArr.push_back(intersect);
				hit = true;
			}
			if (LineSegmentIntersectionPoint(shapeArr[i]->getVertex()[2] + Vector2(1.05, 0), shapeArr[i]->getVertex()[3] + Vector2(-1.05, 0), intersect))
			{
				intersectArr.push_back(intersect);
				hit = true;
			}
			if (LineSegmentIntersectionPoint(shapeArr[i]->getVertex()[3] + Vector2(0, -1.05), shapeArr[i]->getVertex()[0] + Vector2(0, 1.05), intersect))
			{
				intersectArr.push_back(intersect);
				hit = true;
			}
